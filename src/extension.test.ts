...
suite("Syntax Highlighting Integration Tests", () => {
  ...
  test("Semantic tokens highlight file paths in command output", async () => {
    ...
    const command =
      `node -e "console.error('src/main.ts:42:15: error: something went wrong')"`;
    await assertEdit(activeEditor, (editBuilder) => {
      editBuilder.replace(new vscode.Range(0, 0, 0, 0), command);
    });
    ...
    const text = activeEditor.document.getText();
    assert.ok(
      text.includes("src/main.ts:42:15"),
      "Document should contain file path",
    );
    ...
    for (let i = 1; i < activeEditor.document.lineCount; i++) {
      const lineText = activeEditor.document.lineAt(i).text;
      if (
        lineText.includes("src/main.ts:42:15") && lineText.includes("error:") &&
        !lineText.includes("console.error")
      ) { // Exclude the command line
        pathLineIndex = i;
        break;
      }
    }
    assert.ok(
      pathLineIndex >= 0,
      "Should find output line with file path and error",
    );
    ...
    if (tokens) {
      ...
      const pathTokens = validTokens.filter((t) =>
        t.expectedType === "string" && t.line === pathLineIndex
      );
      const pathToken = pathTokens.find((t) =>
        t.tokenText === "src/main.ts:42:15"
      );
      assert.ok(
        pathToken,
        "Should have file path token 'src/main.ts:42:15' on correct line",
      );
      assert.strictEqual(
        pathToken!.length,
        17,
        "Path token should have correct length",
      );

      // Should have an error token on the output line
      const errorTokens = validTokens.filter((t) =>
        t.expectedType === "property" && t.line === pathLineIndex
      );
      ...
      assert.strictEqual(
        errorToken!.length,
        6,
        "Error token should have length 6",
      );
    }
  });
  ...
  test("Semantic tokens correctly map tag types to VSCode token types", async () => {
    ...
    const command =
      `node -e "console.log('Done'); console.error('Warning: test.js:10:5: minor issue')"`;
    await assertEdit(activeEditor, (editBuilder) => {
      editBuilder.replace(new vscode.Range(0, 0, 0, 0), command);
    });
    ...
    assert.ok(
      statusResult.ranges.length > 0,
      "Should have status highlight ranges",
    );

    // Check for specific range types in status
    const punctuationRanges = statusResult.ranges.filter((r) =>
      r.tag === "punctuation"
    );
    const keywordRanges = statusResult.ranges.filter((r) =>
      r.tag === "keyword"
    );
    const timeRanges = statusResult.ranges.filter((r) => r.tag === "time");
    const statusOkRanges = statusResult.ranges.filter((r) =>
      r.tag === "status_ok"
    );

    assert.ok(
      punctuationRanges.length >= 2,
      "Should have punctuation ranges for = symbols",
    );
  ...
  test("Semantic token ranges are precise for complex output with multiple paths and errors", async () => {
    ...
    const command =
      `node -e "console.error('lib/parser.ts:25:3: error: Type error'); console.error('src/utils.js:108:12: error: Name error')"`;
    await assertEdit(activeEditor, (editBuilder) => {
      editBuilder.replace(new vscode.Range(0, 0, 0, 0), command);
    });
    ...
    if (tokens) {
      ...
      expectedPaths.forEach((expectedPath) => {
        ...
        assert.strictEqual(
          pathToken!.length,
          expectedPath.length,
          `Path token should have correct length for '${expectedPath}'`,
        );
      });

      // Should have error tokens
      const errorTokens = validTokens.filter((t) =>
        t.expectedType === "property"
      );
      const errorTokensWithText = errorTokens.filter((t) =>
        t.tokenText === "error:"
      );
      assert.ok(
        errorTokensWithText.length >= 2,
        "Should have at least 2 error tokens",
      );

      // Verify no overlapping tokens - each token should have a unique position
      const tokenPositions = validTokens.map((t) =>
        `${t.line}:${t.character}-${t.character + t.length}`
      );
      const uniquePositions = new Set(tokenPositions);
      assert.strictEqual(
        tokenPositions.length,
        uniquePositions.size,
        "All tokens should have unique positions (no overlaps)",
      );
    }
  });
});
