...
export function activate(context: vscode.ExtensionContext) {
	...
	let executeDisposable = vscode.commands.registerCommand('terminal-editor.execute', async () => {
		const terminalUri = vscode.Uri.parse('terminal-editor:/terminal');
		let terminalEditor: vscode.TextEditor | undefined = undefined;
		for (terminalEditor of vscode.window.visibleTextEditors) {
		// Check if this is the terminal editor
			if (terminalEditor.document.uri.toString() === terminalUri.toString()) break;
		}
		if (!terminalEditor) {
